...
from preprocess.combag import make_quasi_data, get_word2id
from preprocess.pickledata import make_train_data
...
def AUC_and_PN(model, datasets, args):
    ...
    np.save(args.savepath+'/' + model.name + '_true.npy', y_true)
    np.save(args.savepath+'/' + model.name + '_scores.npy', y_scores)
...
def quasiTrainModel(model, train_temp_data, datasets, args):
    sentence_len = 80
    max_filter_len = 3
    num_classes = 53
    group_size = 4
    ...
    batch = args.batch_size_pre
    for epoch in range(args.pretrain_epoch):
        print("epoch : {} of quasitraining ".format(epoch))
        shuffle(train_temp_data)
        quasi_bags = make_quasi_data(train_temp_data, word2id, max_filter_len, sentence_len, num_classes, group_size)
        print("len of quasi_bags:{}".format(len(quasi_bags)))
        print("size of quasi_bags:{}".format(group_size))
        [train_label, train_sents, _, train_ldist, train_rdist, _, train_epos, train_sentlen] = bags_decompose(
            quasi_bags)
        temp_order = list(range(len(train_label)))
        print("len(temp_order):{}".format(len(temp_order)))
